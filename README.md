...
* `molecules_simple`: Simplified version of the `molecules` collection with the following fields excluded: `ref_structure`, `min_h_index_conformers_idxs`, `conformers`, `processing_metadata`, and `valid_conformer_ids`. Also includes two additional fields not found in the `molecules` collection: `log_conformer_density` and `Delta_log_psat` (see Schema Details for more info)
...
<details>
<summary><b>📑 Collection: molecules_simple</b> (Click to expand)</summary>
...
| `psat_COSMO_atm` | Double | Saturation vapor pressure in standard atmospheres calculated by COSMOtherm. |
| `psat_SIMPOL_atm` | Double | Saturation vapor pressure in standard atmospheres calculated by SIMPOL. |
| `log_conformer_density` | Double | A quantity computed as $\log_{10}(\frac{\text{num\_conformers}}{\text{reestimated\_flexibility} \cdot E_H^{-1} + 10^{-6}})$, where $E_H$ is a unit of energy in Hartrees. Describes the molecule's structural configuration entropy, relative to its internal bonding constraints. |
| `Delta_log_psat` | Double | A quantity computed as $\log_{10}(\text{psat\_SIMPOL\_atm}) - \log_{10}(\text{psat\_COSMO\_atm})$. Describes the magnitude difference between the two saturation vapor pressure estimates  |
...
| :--- | :--- | :--- |
| `_id` | ObjectId | Unique identifier for the document generated by MongoDB. |
| `Index` | Int32 | Unique identifier for the product. |
| `Gecko` | String | GECKO-A formula of the product. |
| `SMILES` | String | The corresponding SMILES string for the product. |
| `Dataset` | String | Source dataset (BC, Trep, or DTA) |
| `Gen1` | Int32 | Generation in which the first radical is first produced. (Note that generations are determined by formation of stable closed-shell species. A radical produced from the decomposition of a 1st Gen radical is also a 1st Gen radical.) |
| `Gen2` | Int32 | Generation in which the second radical is first produced. |
| `Parent1` | Array | Precursor molecules of the first radical. |
| `Parent2` | Array | Precursor molecules of the second radical. |
| `Oxidant1` | Array | VOC oxidants ($\text{OH}$, $\text{O}_3$ and $\text{NO}_3$ ) needed for the formation of the first radical. |
| `Oxidant2` | Array | VOC oxidants needed for the formation of the second radical. |
| `mw` | Double | Molecular mass of the product in g/mol |
| `PsatNan` | Double | Nannoolal saturation vapour pressure. |
| `PsatSim` | Double | SIMPOL saturation vapour pressure. |
